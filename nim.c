...
static pthread_mutex_t *i2c_bus_mutex = NULL;
...
/* -------------------------------------------------------------------------------------------------- */
/* Sets the I2C bus mutex for dual-tuner synchronization                                              */
/*   mutex: pointer to the I2C mutex to use, or NULL to disable synchronization                      */
/* -------------------------------------------------------------------------------------------------- */
    i2c_bus_mutex = mutex;
}

/* -------------------------------------------------------------------------------------------------- */
static uint8_t nim_write_demod_unlocked(uint16_t reg, uint8_t val) {
/* -------------------------------------------------------------------------------------------------- */
/* writes to a demodulator register without I2C locking (for internal use)                           */
...
/* -------------------------------------------------------------------------------------------------- */
    ...
    /* Lock I2C bus for dual-tuner synchronization */
    if (i2c_bus_mutex) {
    ...
    if (repeater_on) {
        repeater_on=false;
        err=nim_write_demod_unlocked(0xf12a,0x38);
    }
...
/* -------------------------------------------------------------------------------------------------- */
    ...
    if (!repeater_on) {
        err=nim_write_demod_unlocked(0xf12a,0xb8);
        repeater_on=true;
    }
