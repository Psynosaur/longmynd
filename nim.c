...
bool repeater_on;
bool repeater_on2; // Second tuner repeater state
...
uint8_t nim_send_d0() {
/* -------------------------------------------------------------------------------------------------- */
/* sends a D0 command to the nim (placeholder implementation)                                         */
/* return: error code                                                                                 */
/* -------------------------------------------------------------------------------------------------- */
    // This function appears to be a placeholder or legacy function
    // Implementation would depend on specific hardware requirements
...
/* -------------------------------------------------------------------------------------------------- */
/* initialises a specific tuner's nim                                                                 */
...
/* -------------------------------------------------------------------------------------------------- */
    ...
    /* check we can read and write a register */
    if (err==ERROR_NONE) err=nim_write_demod_tuner(tuner, 0xf536, 0xaa);
    if (err==ERROR_NONE) err=nim_read_demod_tuner(tuner, 0xf536, &val);
    if (err==ERROR_NONE) {
        if (val!=0xaa) {
    ...
    /* we always want to start with the i2c repeater turned off */
    if (err!=ERROR_NONE) err=nim_write_demod_tuner(tuner, 0xf12a, 0x38);
...
/* -------------------------------------------------------------------------------------------------- */
/* reads a demodulator register for specific tuner                                                    */
/*  tuner: tuner number (1 or 2)                                                                      */
/*    reg: which demod register to read                                                               */
/*    val: where to put the result                                                                    */
/* return: error code                                                                                 */
/* -------------------------------------------------------------------------------------------------- */
    ...
    if (tuner == 1) {
        ...
        /* if we are not using the tuner or lna any more then we can turn off the repeater */
        if (repeater_on2) {
            repeater_on2=false;
            err=ftdi_i2c_write_reg16_tuner(2, NIM_DEMOD_ADDR, 0xf12a, 0x38);
        }
        if (err==ERROR_NONE) err=ftdi_i2c_read_reg16_tuner(2, NIM_DEMOD_ADDR, reg, val);
        if (err!=ERROR_NONE) printf("ERROR: tuner2 demod read 0x%.4x\n", reg);
        return err;
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
...
/* -------------------------------------------------------------------------------------------------- */
/* writes to a demodulator register for specific tuner                                                */
/*  tuner: tuner number (1 or 2)                                                                      */
/*    reg: which demod register to write to                                                           */
/*    val: what to write to it                                                                        */
/* return: error code                                                                                 */
/* -------------------------------------------------------------------------------------------------- */
    ...
    if (tuner == 1) {
        ...
        if (err==ERROR_NONE) err=ftdi_i2c_write_reg16_tuner(2, NIM_DEMOD_ADDR, reg, val);
        if (err!=ERROR_NONE) printf("ERROR: tuner2 demod write 0x%.4x, 0x%.2x\n", reg, val);
        return err;
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
...
/* -------------------------------------------------------------------------------------------------- */
/* reads from the tuner for specific tuner device                                                     */
/*  tuner: tuner number (1 or 2)                                                                      */
/*    reg: which tuner register to read from                                                          */
/*    val: where to put the result                                                                    */
/* return: error code                                                                                 */
/* -------------------------------------------------------------------------------------------------- */
    ...
    if (tuner == 1) {
        ...
        if (!repeater_on2) {
            err=ftdi_i2c_write_reg16_tuner(2, NIM_DEMOD_ADDR, 0xf12a, 0xb8);
            repeater_on2=true;
        }
        if (err==ERROR_NONE) err=ftdi_i2c_read_reg8_tuner(2, NIM_TUNER_ADDR, reg, val);
        if (err!=ERROR_NONE) printf("ERROR: tuner2 tuner read 0x%.2x\n", reg);
        return err;
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
...
/* -------------------------------------------------------------------------------------------------- */
/* writes to the tuner for specific tuner device                                                      */
/*  tuner: tuner number (1 or 2)                                                                      */
/*    reg: which tuner register to write to                                                           */
/*    val: what to write to it                                                                        */
/* return: error code                                                                                 */
/* -------------------------------------------------------------------------------------------------- */
    ...
    if (tuner == 1) {
        ...
        if (err==ERROR_NONE) err=ftdi_i2c_write_reg8_tuner(2, NIM_TUNER_ADDR, reg, val);
        if (err!=ERROR_NONE) printf("ERROR: tuner2 tuner write %i,%i\n", reg, val);
        return err;
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
...
/* -------------------------------------------------------------------------------------------------- */
/* reads from the specified lna for specific tuner                                                    */
/*    tuner: tuner number (1 or 2)                                                                    */
/* lna_addr: i2c address of the lna to access                                                         */
/*      reg: which lna register to read                                                               */
/*      val: where to put the result                                                                  */
...
/* -------------------------------------------------------------------------------------------------- */
    ...
    if (tuner == 1) {
        ...
        if (err==ERROR_NONE) err=ftdi_i2c_read_reg8_tuner(2, lna_addr, reg, val);
        if (err!=ERROR_NONE) printf("ERROR: tuner2 lna read 0x%.2x, 0x%.2x\n", lna_addr, reg);
        return err;
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
...
/* -------------------------------------------------------------------------------------------------- */
/* writes to the specified lna for specific tuner                                                     */
/*    tuner: tuner number (1 or 2)                                                                    */
/* lna_addr: i2c address of the lna to access                                                         */
/*      reg: which lna register to write to                                                           */
/*      val: what to write to it                                                                      */
...
/* -------------------------------------------------------------------------------------------------- */
    ...
    if (tuner == 1) {
        ...
        if (err==ERROR_NONE) err=ftdi_i2c_write_reg8_tuner(2, lna_addr, reg, val);
        if (err!=ERROR_NONE) printf("ERROR: tuner2 lna write 0x%.2x, 0x%.2x,0x%.2x\n", lna_addr, reg, val);
        return err;
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
