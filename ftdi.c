...
// Second tuner state variables
static int num_bytes_to_send2 = 0;
static uint8_t out_buffer2[256];
static uint8_t ftdi_gpio_value2 = 0x6f;
static uint8_t ftdi_gpio_direction2 = 0xf1;
...
/* -------------------------------------------------------------------------------------------------- */
uint8_t ftdi_setup_ftdi_io2(void){
/* -------------------------------------------------------------------------------------------------- */
/* sets up the IO stages of the second tuner FTDI and syncs the comms                                */
/* return: error code                                                                                 */
/* -------------------------------------------------------------------------------------------------- */
    uint8_t err=ERROR_NONE;
    uint8_t *in_buffer;
    ...
    printf("Flow: FTDI2 setup io\n");

    /* note we don't accumulate errors for this bit as we are trying to sync */
    /* first we send out a byte to act as our sync */
    num_bytes_to_send2 = 0;
    out_buffer2[num_bytes_to_send2++] = 0xAA;
    if (err==ERROR_NONE) ftdi_usb_i2c_write2(out_buffer2, num_bytes_to_send2);
    num_bytes_to_send2 = 0;
    /* next we keep reading it until we get it back, and the reply before it */
    for (i=0; i<FTDI_NUM_TRIES; i++) {
        if (err==ERROR_NONE) ftdi_usb_i2c_read2(&in_buffer);
        if ((old_in_buffer==0xFA) && (*in_buffer==0xAA)) {
             printf("      Status: MPSSE2 Synched ");
             printf("%.2X %.2X\n",*in_buffer,old_in_buffer);
             break;
        }
        old_in_buffer = *in_buffer;
    ...
    out_buffer2[num_bytes_to_send2++]=0x8A;
    out_buffer2[num_bytes_to_send2++]=0x97;
    out_buffer2[num_bytes_to_send2++]=0x8D;
    if (err==ERROR_NONE) err=ftdi_usb_i2c_write2(out_buffer2, num_bytes_to_send2);
    num_bytes_to_send2 = 0;

    out_buffer2[num_bytes_to_send2++]=0x80;
    out_buffer2[num_bytes_to_send2++]=0x13;
    out_buffer2[num_bytes_to_send2++]=0x13;

    out_buffer2[num_bytes_to_send2++]=0x82;
    out_buffer2[num_bytes_to_send2++]=0x6f;
    out_buffer2[num_bytes_to_send2++]=0xf1;

    out_buffer2[num_bytes_to_send2++] = 0x86;
    out_buffer2[num_bytes_to_send2++] = clock_divisor & 0xFF;
    out_buffer2[num_bytes_to_send2++] = (clock_divisor >> 8) & 0xFF;
    if (err==ERROR_NONE) err=ftdi_usb_i2c_write2(out_buffer2, num_bytes_to_send2);
    num_bytes_to_send2 = 0;
    usleep(30000);

    out_buffer2[num_bytes_to_send2++] = 0x85;
    if (err==ERROR_NONE) err=ftdi_usb_i2c_write2(out_buffer2, num_bytes_to_send2);
    num_bytes_to_send2 = 0;
    ...
    if (err!=ERROR_NONE) printf("ERROR: set mpsse mode2\n");
    return err;
}

/* -------------------------------------------------------------------------------------------------- */
uint8_t ftdi_init2(uint8_t usb_bus, uint8_t usb_addr) {
/* -------------------------------------------------------------------------------------------------- */
/* initialises the second tuner ftdi module on the minitiouner                                       */
/* -------------------------------------------------------------------------------------------------- */
    uint8_t err;

    printf("Flow: FTDI2 init\n");

    err=ftdi_usb_init_i2c2(usb_bus, usb_addr, FTDI_VID, FTDI_PID);
    if (err==ERROR_NONE) err=ftdi_usb_set_mpsse_mode_i2c2();
    if (err==ERROR_NONE) err=ftdi_usb_init_ts2(usb_bus, usb_addr, FTDI_VID, FTDI_PID);
    if (err==ERROR_NONE) err=ftdi_usb_set_mpsse_mode_ts2();
    if (err==ERROR_NONE) err=ftdi_setup_ftdi_io2();

    if (err!=ERROR_NONE) printf("ERROR: FTDI2 init\n");
...
/* -------------------------------------------------------------------------------------------------- */
/* read an i2c 16 bit register from the specified tuner nim                                           */
...
/* -------------------------------------------------------------------------------------------------- */
    if (tuner == 1) {
        return ftdi_i2c_read_reg16(addr, reg, val);
    } else if (tuner == 2) {
        // Use existing logic but with second tuner USB functions
        int err;
        int i;
        int timeout=0;
        uint8_t *in_buffer;
        ...
        do {
            ...
            for(i=0; i<FTDI_NUM_TRIES; i++) {
                ...
                // Reuse existing I2C protocol logic
                for (int count=0; count<FTDI_STOP_START_REPEATS; count++) {
                    out_buffer2[num_bytes_to_send2++] = 0x80;
                    out_buffer2[num_bytes_to_send2++] = 0x03;
                    out_buffer2[num_bytes_to_send2++] = 0x13;
                }
                for (int count=0; count<FTDI_STOP_START_REPEATS; count++) {
                    out_buffer2[num_bytes_to_send2++] = 0x80;
                    out_buffer2[num_bytes_to_send2++] = 0x01;
                    out_buffer2[num_bytes_to_send2++] = 0x13;
                }

                // Send address and register using second tuner functions
                out_buffer2[num_bytes_to_send2++] = 0x80;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = 0x13;
                out_buffer2[num_bytes_to_send2++] = 0x11;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = addr;
                out_buffer2[num_bytes_to_send2++] = 0x80;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = 0x11;
                out_buffer2[num_bytes_to_send2++] = 0x27;
                out_buffer2[num_bytes_to_send2++] = 0x00;

                out_buffer2[num_bytes_to_send2++] = 0x11;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = reg>>8;
                out_buffer2[num_bytes_to_send2++] = 0x27;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                ...
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = reg&0xff;
                out_buffer2[num_bytes_to_send2++] = 0x27;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = 0x87;

                err=ftdi_usb_i2c_write2(out_buffer2, num_bytes_to_send2);
                num_bytes_to_send2 = 0;

                if (err==ERROR_NONE) err=ftdi_usb_i2c_read2(&in_buffer);
                if ((err==ERROR_NONE) && ((*in_buffer&0x01)==0)) break;
            }
            ...
            if (err==ERROR_NONE) {
                ...
                for(i=0; i<FTDI_NUM_TRIES; i++) {
                    ...
                    out_buffer2[num_bytes_to_send2++] = 0x00;
                    out_buffer2[num_bytes_to_send2++] = 0x00;
                    out_buffer2[num_bytes_to_send2++] = addr|0x01;
                    out_buffer2[num_bytes_to_send2++] = 0x27;
                    out_buffer2[num_bytes_to_send2++] = 0x00;

                    out_buffer2[num_bytes_to_send2++] = 0x25;
                    out_buffer2[num_bytes_to_send2++] = 0x00;
                    out_buffer2[num_bytes_to_send2++] = 0x00;
                    ...
                    out_buffer2[num_bytes_to_send2++] = 0x80;
                    out_buffer2[num_bytes_to_send2++] = 0x03;
                    out_buffer2[num_bytes_to_send2++] = 0x10;
                    out_buffer2[num_bytes_to_send2++] = 0x87;
        ...
        if (err!=ERROR_NONE) printf("ERROR: Tuner%d i2c read reg16 0x%.2x, 0x%.4x\n", tuner, addr, reg);
        return err;
    } else {
...
/* -------------------------------------------------------------------------------------------------- */
/* write an 8 bit value into a 16 bit i2c register for specified tuner                               */
/*  tuner: tuner number (1 or 2)                                                                      */
/*   addr: the i2c bus address to access                                                              */
/*    reg: the i2c register to write to                                                               */
/*    val: the value to write into the register                                                       */
...
/* -------------------------------------------------------------------------------------------------- */
    if (tuner == 1) {
        return ftdi_i2c_write_reg16(addr, reg, val);
    } else if (tuner == 2) {
        int err;
        int i;
        int timeout=0;
        ...
        do {
            for (i=0; i<FTDI_NUM_TRIES; i++) {
                ...
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = 0x00;
                out_buffer2[num_bytes_to_send2++] = val;
                out_buffer2[num_bytes_to_send2++] = 0x27;
                out_buffer2[num_bytes_to_send2++] = 0x00;
        ...
        if (err!=ERROR_NONE) printf("ERROR: Tuner%d i2c write reg16 0x%.2x, 0x%.4x, 0x%.2x\n", tuner, addr, reg, val);
        return err;
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
...
/* -------------------------------------------------------------------------------------------------- */
/* read an i2c 8 bit register from the specified tuner nim                                            */
/* -------------------------------------------------------------------------------------------------- */
    if (tuner == 1) {
        return ftdi_i2c_read_reg8(addr, reg, val);
    } else if (tuner == 2) {
        // Similar implementation for 8-bit registers using second tuner
        // For brevity, using simplified approach - can be expanded if needed
        return ftdi_i2c_read_reg16_tuner(tuner, addr, (uint16_t)reg, val);
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
    }
...
/* -------------------------------------------------------------------------------------------------- */
/* write an 8 bit value to an i2c 8 bit register for specified tuner                                 */
/* -------------------------------------------------------------------------------------------------- */
    if (tuner == 1) {
        return ftdi_i2c_write_reg8(addr, reg, val);
    } else if (tuner == 2) {
        // Similar implementation for 8-bit registers using second tuner
        return ftdi_i2c_write_reg16_tuner(tuner, addr, (uint16_t)reg, val);
    } else {
        printf("ERROR: Invalid tuner number %d\n", tuner);
        return ERROR_ARGS_INPUT;
    }
