...
/* -------------------------------------------------------------------------------------------------- */
/* minimal initialization for second FTDI device - only TS streaming, no I2C setup                  */
/* I2C communication will use the first tuner's connection for both TOP and BOTTOM demodulator paths */
/* -------------------------------------------------------------------------------------------------- */
    uint8_t err;

    printf("Flow: FTDI init2 (minimal - TS streaming only)\n");

    // Only initialize TS streaming for second tuner
    err=ftdi_usb_init_ts2(usb_bus, usb_addr, FTDI_VID, FTDI_PID);
    if (err==ERROR_NONE) err=ftdi_usb_set_mpsse_mode_ts2();

    if (err!=ERROR_NONE) printf("ERROR: FTDI init2\n");
    else printf("Flow: Second FTDI device initialized for TS streaming only\n");
