...
/* -------------------------------------------------------------------------------------------------- */
uint8_t stv6120_powerdown_single_path(uint8_t tuner) {
/* -------------------------------------------------------------------------------------------------- */
/* Powers down a specific tuner signal path (synthesizer, mixer, gain amplifier)                     */
/*   tuner: TUNER_1  |  TUNER_2 : which tuner path to power down                                     */
/*  return: error code                                                                                */
/* -------------------------------------------------------------------------------------------------- */
    uint8_t err = ERROR_NONE;

    printf("Flow: Powering down stv6120 signal path for tuner %d\n", tuner + 1);

    if (tuner == TUNER_1) {
        /* Power down Path 1 only */
        err = stv6120_write_reg(STV6120_CTRL2,
            (STV6120_CTRL2_DCLOOPOFF_DISABLE << STV6120_CTRL2_DCLOOPOFF_SHIFT) |
            (STV6120_CTRL2_SDOFF_ON          << STV6120_CTRL2_SDOFF_SHIFT)     |
            (STV6120_CTRL2_SYN_OFF           << STV6120_CTRL2_SYN_SHIFT)       |
            (STV6120_CTRL2_REFOUTSEL_1_25V   << STV6120_CTRL2_REFOUTSEL_SHIFT) |
            (STV6120_CTRL2_BBGAIN_0DB        << STV6120_CTRL2_BBGAIN_SHIFT)    );
    } else if (tuner == TUNER_2) {
        /* Power down Path 2 only */
        err = stv6120_write_reg(STV6120_CTRL11,
            (STV6120_CTRL2_DCLOOPOFF_DISABLE << STV6120_CTRL2_DCLOOPOFF_SHIFT) |
            (STV6120_CTRL2_SDOFF_ON          << STV6120_CTRL2_SDOFF_SHIFT)     |
            (STV6120_CTRL2_SYN_OFF           << STV6120_CTRL2_SYN_SHIFT)       |
            (STV6120_CTRL2_REFOUTSEL_1_25V   << STV6120_CTRL2_REFOUTSEL_SHIFT) |
